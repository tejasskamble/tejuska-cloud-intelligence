...
if not st.session_state.authenticated:
    ...
    with tab2:
        with st.form("signup_form"):
            ...
            if st.form_submit_button("Create Account", type="primary", use_container_width=True):
                if first_name and last_name and contact and email and password and "@" in email:
                    # Simulate successful registration
                    st.success("Account created! Please sign in.")
                else:
                    st.error("All fields are required.")

    # OAuth buttons (sleek, horizontal, with icons)
    st.markdown(
        """
        <div class="flex items-center my-6">
            <div class="flex-1 border-t border-slate-300 dark:border-slate-700"></div>
            <span class="px-4 opacity-50">OR CONTINUE WITH</span>
            <div class="flex-1 border-t border-slate-300 dark:border-slate-700"></div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with col1:
        st.markdown(
            """
            <button class="flex items-center justify-center w-full gap-2 px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800">
    ...
    st.markdown(
        f'<div class="text-center py-8"><p class="text-xl">Authenticated as: {st.session_state.tenant_id}</p><p class="opacity-70">Expand the sidebar to access the FinOps Dashboard, Automations, AI Assistant, and more.</p></div>',
        unsafe_allow_html=True,
    )
